...
pub fn main() !void {
    ...
    const zi = try s.allocateIndex(&root.cert_id, "zone", 6);
    const zone = try identity.deriveChildIdentity(a, EMAIL, SALT, "root", "zone", 6, zi);
    defer zone.deinit(a);
    try s.putNode(.{ .cert_id = &zone.cert_id, .parent_cert_id = &root.cert_id, .resource_id = "zone", .domain_flag = 6, .child_index = zi, .label = "north" });

    for (0..3) |_| {
        const di = try s.allocateIndex(&zone.cert_id, "device", 6);
        const unit = try identity.deriveChildIdentity(a, EMAIL, SALT, zone.derivation_path, "device", 6, di);
        defer unit.deinit(a);
        try s.putNode(.{ .cert_id = &unit.cert_id, .parent_cert_id = &zone.cert_id, .resource_id = "device", .domain_flag = 6, .child_index = di, .label = "unit" });
    }
    _ = try s.burnSlot(&zone.cert_id, "device", 6); // a retired index to carry
