// bitpiggy_mesh — BitPiggy chore-claim demo on the existing cell-mesh substrate.
//
// Each XIAO ESP32-C6:
...
static const char *TAG = "bitpiggy_mesh";

// ── Hardware pins ────────────────────────────────────────────────────
//
// The original demo targeted XIAO ESP32-C6, where GPIO15 is the active-low
// onboard LED. LilyGO T-Display-S3 uses GPIO15 as display/V3V power enable;
// driving it low makes the screen look dead while the firmware keeps running.
#if CONFIG_IDF_TARGET_ESP32S3
#define BITPIGGY_TDISPLAY_S3 1
#else
#define BITPIGGY_TDISPLAY_S3 0
#endif

#if !BITPIGGY_TDISPLAY_S3
...
static const char BITPIGGY_CHORE_CLAIM_TYPE_NAME[] = "bitpiggy.chore_claim.v0";
static const char BITPIGGY_APPROVAL_TYPE_NAME[]    = "bitpiggy.approval.v0";
static const char BITPIGGY_REJECTION_TYPE_NAME[]   = "bitpiggy.rejection.v0";
...
static uint8_t    s_bitpiggy_chore_claim_type_hash[32];
static uint8_t    s_bitpiggy_approval_type_hash[32];
static uint8_t    s_bitpiggy_rejection_type_hash[32];
...
typedef enum {
    ...
    DECK_KIND_BITPIGGY_CHORE_CLAIM = 32,
} deck_kind_t;
...
static deck_queue_t s_q_bitpiggy_chore_claim;
...
// ── BitPiggy child-device state ─────────────────────────────────────
// v0 demo state: one pending chore claim at a time. Parent decision cells are
// accepted only after the normal wallet-signature verification path, and only
// if their claim_id references this pending claim.
static uint8_t  s_bitpiggy_pending_claim_id[16];
static bool     s_bitpiggy_has_pending_claim = false;
static uint32_t s_bitpiggy_approved_count = 0;
static uint32_t s_bitpiggy_rejected_count = 0;
...
// LilyGO T-Display-S3, ST7789 170x320 over 8-bit Intel 8080 bus.
// Pinout mirrors LilyGO's Arduino_GFXDemo / TFT_eSPI Setup206.
#define LCD_PWR_GPIO      GPIO_NUM_15
#define LCD_BL_GPIO       GPIO_NUM_38
#define LCD_RST_GPIO      GPIO_NUM_5
#define LCD_CS_GPIO       GPIO_NUM_6
#define LCD_DC_GPIO       GPIO_NUM_7
#define LCD_WR_GPIO       GPIO_NUM_8
#define LCD_RD_GPIO       GPIO_NUM_9
#define LCD_D0_GPIO       GPIO_NUM_39
#define LCD_D1_GPIO       GPIO_NUM_40
#define LCD_D2_GPIO       GPIO_NUM_41
#define LCD_D3_GPIO       GPIO_NUM_42
#define LCD_D4_GPIO       GPIO_NUM_45
#define LCD_D5_GPIO       GPIO_NUM_46
#define LCD_D6_GPIO       GPIO_NUM_47
#define LCD_D7_GPIO       GPIO_NUM_48
...
typedef enum {
    ...
    LCD_STATUS_APPROVED,
    LCD_STATUS_REJECTED,
} bitpiggy_lcd_status_t;

static uint32_t s_lcd_data_clr_mask = 0;
static uint32_t s_lcd_data_set_mask[256];
static bool s_lcd_ready = false;
static volatile bool s_lcd_dirty = true;
static bitpiggy_lcd_status_t s_lcd_status = LCD_STATUS_BOOT;
static uint32_t s_lcd_claim_counter = 0;
static char s_lcd_claim_prefix[9] = "--------";
static uint64_t s_lcd_last_render_us = 0;
...
#define LCD_C_PAPER    rgb565(255, 250, 243)
#define LCD_C_PAPER2   rgb565(247, 239, 225)
#define LCD_C_LINE     rgb565(217, 205, 181)
#define LCD_C_INK      rgb565(31, 42, 36)
#define LCD_C_INK2     rgb565(42, 74, 58)
#define LCD_C_MUTED    rgb565(125, 138, 130)
#define LCD_C_PEACH    rgb565(232, 168, 124)
#define LCD_C_PEACH_D  rgb565(217, 117, 89)
#define LCD_C_LEAF     rgb565(110, 138, 106)
#define LCD_C_SKY      rgb565(138, 169, 181)
#define LCD_C_GOLD     rgb565(217, 176, 112)
#define LCD_C_WHITE    rgb565(255, 255, 255)
...
static uint8_t glyph5x7(char ch, int row) {
    ...
    switch (ch) {
        ...
        case '#': { static const uint8_t r[7] = {0x0A,0x0A,0x1F,0x0A,0x1F,0x0A,0x0A}; return r[row]; }
        case '$': { static const uint8_t r[7] = {0x04,0x0F,0x14,0x0E,0x05,0x1E,0x04}; return r[row]; }
        case '.': { static const uint8_t r[7] = {0x00,0x00,0x00,0x00,0x00,0x0C,0x0C}; return r[row]; }
        case ':': { static const uint8_t r[7] = {0x00,0x0C,0x0C,0x00,0x0C,0x0C,0x00}; return r[row]; }
        case '-': { static const uint8_t r[7] = {0x00,0x00,0x00,0x1F,0x00,0x00,0x00}; return r[row]; }
        default: return 0;
    }
}

#define LCD_CS_MASK (1U << (uint32_t)LCD_CS_GPIO)
#define LCD_DC_MASK (1U << (uint32_t)LCD_DC_GPIO)
#define LCD_WR_MASK (1U << (uint32_t)LCD_WR_GPIO)

static uint32_t lcd_hi_gpio_mask(gpio_num_t gpio) {
    return 1U << ((uint32_t)gpio - 32U);
}

static inline void lcd_cs_low(void)  { REG_WRITE(GPIO_OUT_W1TC_REG, LCD_CS_MASK); }
static inline void lcd_cs_high(void) { REG_WRITE(GPIO_OUT_W1TS_REG, LCD_CS_MASK); }
static inline void lcd_dc_low(void)  { REG_WRITE(GPIO_OUT_W1TC_REG, LCD_DC_MASK); }
static inline void lcd_dc_high(void) { REG_WRITE(GPIO_OUT_W1TS_REG, LCD_DC_MASK); }

static inline void lcd_wr_pulse(void) {
    REG_WRITE(GPIO_OUT_W1TC_REG, LCD_WR_MASK);
    REG_WRITE(GPIO_OUT_W1TS_REG, LCD_WR_MASK);
}

static inline void lcd_write8(uint8_t data) {
    REG_WRITE(GPIO_OUT1_W1TC_REG, s_lcd_data_clr_mask);
    REG_WRITE(GPIO_OUT1_W1TS_REG, s_lcd_data_set_mask[data]);
    lcd_wr_pulse();
}
...
static void lcd_set_window_begin_pixels(int x, int y, int w, int h) {
    uint16_t x0 = (uint16_t)(x + 35);
    uint16_t x1 = (uint16_t)(x + w - 1 + 35);
    ...
    lcd_write_cmd_data(0x2A, caset, sizeof(caset));
    lcd_write_cmd_data(0x2B, raset, sizeof(raset));
    lcd_cs_low();
    lcd_dc_low();
    lcd_write8(0x2C);
    lcd_dc_high();
...
static void lcd_write_repeat_color(uint16_t color, uint32_t count) {
    uint8_t hi = (uint8_t)(color >> 8);
    uint8_t lo = (uint8_t)color;
    if (hi == lo) {
        REG_WRITE(GPIO_OUT1_W1TC_REG, s_lcd_data_clr_mask);
        REG_WRITE(GPIO_OUT1_W1TS_REG, s_lcd_data_set_mask[hi]);
        for (uint32_t i = 0; i < count * 2U; i++) {
            lcd_wr_pulse();
        }
        return;
    }
    uint32_t hi_mask = s_lcd_data_set_mask[hi];
    uint32_t lo_mask = s_lcd_data_set_mask[lo];
    for (uint32_t i = 0; i < count; i++) {
        REG_WRITE(GPIO_OUT1_W1TC_REG, s_lcd_data_clr_mask);
        REG_WRITE(GPIO_OUT1_W1TS_REG, hi_mask);
        lcd_wr_pulse();
        REG_WRITE(GPIO_OUT1_W1TC_REG, s_lcd_data_clr_mask);
        REG_WRITE(GPIO_OUT1_W1TS_REG, lo_mask);
        lcd_wr_pulse();
    }
}
...
static void lcd_fill_rect(int x, int y, int w, int h, uint16_t color) {
    ...
    lcd_set_window_begin_pixels(x, y, w, h);
    lcd_write_repeat_color(color, (uint32_t)w * (uint32_t)h);
    lcd_cs_high();
}

static void lcd_draw_char(int x, int y, char ch, uint16_t fg, uint16_t bg, int scale) {
    if (!s_lcd_ready || scale < 1) return;
    const int cw = 6 * scale;
    const int chh = 8 * scale;
    if (x < 0 || y < 0 || x + cw > LCD_H_RES || y + chh > LCD_V_RES) return;

    lcd_set_window_begin_pixels(x, y, cw, chh);
    for (int py = 0; py < chh; py++) {
        int gy = py / scale;
        uint8_t bits = (gy < 7) ? glyph5x7(ch, gy) : 0;
        for (int px = 0; px < cw; px++) {
            int gx = px / scale;
            bool on = (gx < 5) && ((bits & (1u << (4 - gx))) != 0);
            lcd_write_color(on ? fg : bg);
        }
    }
    lcd_cs_high();
}
...
static void lcd_draw_text(int x, int y, const char *text, uint16_t fg, uint16_t bg, int scale) {
    ...
    for (const char *p = text; *p; p++) {
        lcd_draw_char(cx, y, *p, fg, bg, scale);
        cx += 6 * scale;
    }
}

static void lcd_draw_centered(int y, const char *text, uint16_t fg, uint16_t bg, int scale) {
    int len = (int)strlen(text);
    int x = (LCD_H_RES - len * 6 * scale) / 2;
    if (x < 0) x = 0;
    lcd_draw_text(x, y, text, fg, bg, scale);
}

static void lcd_reference_init_sequence(void) {
    // LilyGO's Arduino_GFX example applies these ST7789 panel-power/gamma
    // commands after reset. The generic IDF ST7789 init is intentionally
    // minimal and can leave this 170x320 module in a dim/blank-looking state.
    lcd_write_cmd(0x01); // software reset
    vTaskDelay(pdMS_TO_TICKS(120));
    lcd_write_cmd(0x11); // sleep out
    vTaskDelay(pdMS_TO_TICKS(120));
    lcd_write_cmd_data(0x3A, (const uint8_t[]){0x55}, 1); // RGB565
    lcd_write_cmd_data(0x36, (const uint8_t[]){0x00}, 1); // MADCTL
    lcd_write_cmd_data(0xB0, (const uint8_t[]){0x00, 0xF0}, 2);
    lcd_write_cmd_data(0xB2, (const uint8_t[]){0x0C, 0x0C, 0x00, 0x33, 0x33}, 5);
    lcd_write_cmd_data(0xB7, (const uint8_t[]){0x35}, 1);
    lcd_write_cmd_data(0xBB, (const uint8_t[]){0x19}, 1);
    lcd_write_cmd_data(0xC0, (const uint8_t[]){0x2C}, 1);
    lcd_write_cmd_data(0xC2, (const uint8_t[]){0x01}, 1);
    lcd_write_cmd_data(0xC3, (const uint8_t[]){0x12}, 1);
    lcd_write_cmd_data(0xC4, (const uint8_t[]){0x20}, 1);
    lcd_write_cmd_data(0xC6, (const uint8_t[]){0x0F}, 1);
    lcd_write_cmd_data(0xD0, (const uint8_t[]){0xA4, 0xA1}, 2);
    lcd_write_cmd_data(0xE0, (const uint8_t[]){
        0xF0, 0x09, 0x13, 0x12, 0x12, 0x2B, 0x3C,
        0x44, 0x4B, 0x1B, 0x18, 0x17, 0x1D, 0x21
    }, 14);
    lcd_write_cmd_data(0xE1, (const uint8_t[]){
        0xF0, 0x09, 0x13, 0x0C, 0x0D, 0x27, 0x3B,
        0x44, 0x4D, 0x0B, 0x17, 0x17, 0x1D, 0x21
    }, 14);
    lcd_write_cmd(0x21); // display inversion on
    lcd_write_cmd(0x13); // normal display mode
    vTaskDelay(pdMS_TO_TICKS(10));
    lcd_write_cmd(0x29); // display on
    vTaskDelay(pdMS_TO_TICKS(20));
}

static void lcd_render_self_test(void) {
    lcd_fill_rect(0, 0, LCD_H_RES, 80, LCD_C_PEACH_D);
    lcd_fill_rect(0, 80, LCD_H_RES, 80, LCD_C_LEAF);
    lcd_fill_rect(0, 160, LCD_H_RES, 80, LCD_C_SKY);
    lcd_fill_rect(0, 240, LCD_H_RES, 80, LCD_C_GOLD);
    lcd_draw_centered(22, "BITPIGGY", LCD_C_WHITE, LCD_C_PEACH_D, 2);
    lcd_draw_centered(108, "DISPLAY", LCD_C_WHITE, LCD_C_LEAF, 2);
    lcd_draw_centered(188, "TEST", LCD_C_WHITE, LCD_C_SKY, 2);
    lcd_draw_centered(274, "S3", LCD_C_INK, LCD_C_GOLD, 2);
    lcd_wait_idle();
}

static void lcd_status_set(bitpiggy_lcd_status_t status, uint32_t claim_counter, const uint8_t *claim_id) {
    s_lcd_status = status;
    if (claim_counter != 0) s_lcd_claim_counter = claim_counter;
    if (claim_id) {
        snprintf(s_lcd_claim_prefix, sizeof(s_lcd_claim_prefix),
                 "%02X%02X%02X%02X", claim_id[0], claim_id[1], claim_id[2], claim_id[3]);
    }
    s_lcd_dirty = true;
}

static const char *lcd_status_label(uint16_t *tone) {
    switch (s_lcd_status) {
        case LCD_STATUS_CLAIM:    *tone = LCD_C_PEACH_D; return "CLAIM";
        case LCD_STATUS_APPROVED: *tone = LCD_C_LEAF;    return "APPROVED";
        case LCD_STATUS_REJECTED: *tone = LCD_C_GOLD;    return "NOT YET";
        case LCD_STATUS_READY:    *tone = LCD_C_SKY;     return "READY";
        case LCD_STATUS_BOOT:
        default:                  *tone = LCD_C_MUTED;   return "BOOTING";
    }
}

static void lcd_render(void) {
    if (!s_lcd_ready) return;
    uint16_t tone = LCD_C_SKY;
    const char *status = lcd_status_label(&tone);
    char line[32];

    lcd_fill_rect(0, 0, LCD_H_RES, LCD_V_RES, LCD_C_PAPER);
    lcd_fill_rect(0, 0, LCD_H_RES, 46, LCD_C_INK2);
    lcd_draw_centered(8, "BITPIGGY", LCD_C_WHITE, LCD_C_INK2, 2);
    lcd_draw_centered(30, "CHILD UNIT", LCD_C_PEACH, LCD_C_INK2, 1);

    lcd_fill_rect(10, 58, LCD_H_RES - 20, 42, tone);
    lcd_draw_centered(69, status, LCD_C_WHITE, tone, 2);

    lcd_draw_text(14, 116, "THEO", LCD_C_LEAF, LCD_C_PAPER, 2);
    lcd_draw_text(14, 142, "BINS OUT", LCD_C_INK, LCD_C_PAPER, 2);

    snprintf(line, sizeof(line), "CLAIM #%lu", (unsigned long)s_lcd_claim_counter);
    lcd_draw_text(14, 180, line, LCD_C_INK2, LCD_C_PAPER, 1);
    lcd_draw_text(14, 198, s_lcd_claim_prefix, LCD_C_MUTED, LCD_C_PAPER, 1);

    snprintf(line, sizeof(line), "OK %lu  NO %lu",
             (unsigned long)s_bitpiggy_approved_count,
             (unsigned long)s_bitpiggy_rejected_count);
    lcd_draw_text(14, 228, line, LCD_C_INK2, LCD_C_PAPER, 1);

    snprintf(line, sizeof(line), "TX %lu RX %lu",
             (unsigned long)s_tx_tap_counter,
             (unsigned long)s_rx_counter);
    lcd_draw_text(14, 246, line, LCD_C_MUTED, LCD_C_PAPER, 1);

    lcd_fill_rect(10, 284, LCD_H_RES - 20, 22, LCD_C_PAPER2);
    lcd_draw_centered(291, s_my_mac_str[0] ? &s_my_mac_str[9] : "USB", LCD_C_INK2, LCD_C_PAPER2, 1);

    lcd_wait_idle();
    s_lcd_last_render_us = esp_timer_get_time();
    s_lcd_dirty = false;
}

static void lcd_tick(uint64_t now_us) {
    if (!s_lcd_ready) return;
    if (s_lcd_dirty || now_us - s_lcd_last_render_us > 2000000ULL) {
        lcd_render();
    }
}

static void bitpiggy_display_init(void) {
    const gpio_num_t out_pins[] = {
        LCD_PWR_GPIO, LCD_BL_GPIO, LCD_RST_GPIO, LCD_CS_GPIO, LCD_DC_GPIO,
        LCD_WR_GPIO, LCD_RD_GPIO, LCD_D0_GPIO, LCD_D1_GPIO, LCD_D2_GPIO,
        LCD_D3_GPIO, LCD_D4_GPIO, LCD_D5_GPIO, LCD_D6_GPIO, LCD_D7_GPIO,
    };
    uint64_t out_mask = 0;
    for (size_t i = 0; i < sizeof(out_pins) / sizeof(out_pins[0]); i++) {
        out_mask |= 1ULL << (uint32_t)out_pins[i];
    }
    gpio_config_t cfg = {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "lcd: gpio config failed: %s", esp_err_to_name(err));
        return;
    }
    ...
    s_lcd_data_clr_mask = 0;
    for (size_t i = 0; i < sizeof(data_pins) / sizeof(data_pins[0]); i++) {
        s_lcd_data_clr_mask |= lcd_hi_gpio_mask(data_pins[i]);
        (void)gpio_set_drive_capability(data_pins[i], GPIO_DRIVE_CAP_3);
    }
    for (int value = 0; value < 256; value++) {
        uint32_t set_mask = 0;
        for (int bit = 0; bit < 8; bit++) {
            if (value & (1 << bit)) {
                set_mask |= lcd_hi_gpio_mask(data_pins[bit]);
            }
        }
        s_lcd_data_set_mask[value] = set_mask;
    }
    (void)gpio_set_drive_capability(LCD_WR_GPIO, GPIO_DRIVE_CAP_3);
    ...
    lcd_reference_init_sequence();
    gpio_set_level(LCD_BL_GPIO, 1);

    s_lcd_ready = true;
    lcd_status_set(LCD_STATUS_BOOT, 0, NULL);
    lcd_render_self_test();
    vTaskDelay(pdMS_TO_TICKS(1500));
    lcd_render();
    ESP_LOGI(TAG, "lcd: LilyGO T-Display-S3 direct status panel initialized");
}
...
static inline void bitpiggy_display_init(void) {}
static inline void lcd_status_set(int status, uint32_t claim_counter, const uint8_t *claim_id) {
    (void)status;
    (void)claim_counter;
    (void)claim_id;
}
static inline void lcd_tick(uint64_t now_us) { (void)now_us; }
#define LCD_STATUS_BOOT     0
#define LCD_STATUS_READY    1
#define LCD_STATUS_CLAIM    2
#define LCD_STATUS_APPROVED 3
#define LCD_STATUS_REJECTED 4
...
static bool handle_bitpiggy_decision(bool approved,
                                     const uint8_t *payload,
                                     uint32_t ptot,
                                     const char *source) {
    if (ptot < 32) {
        ESP_LOGW(TAG, "BitPiggy decision [%s]: payload too short (%u)",
                 source, (unsigned)ptot);
        return true;
    }

    const uint8_t *claim_id = payload + 16; // Approval/Rejection.OFF_CLAIM_ID
    if (!s_bitpiggy_has_pending_claim ||
        memcmp(claim_id, s_bitpiggy_pending_claim_id, 16) != 0) {
        ESP_LOGW(TAG, "BitPiggy decision [%s]: no matching pending claim for %02x%02x%02x%02x...",
                 source, claim_id[0], claim_id[1], claim_id[2], claim_id[3]);
        return true;
    }

    if (approved) {
        s_bitpiggy_approved_count++;
        ESP_LOGI(TAG, "*** BITPIGGY APPROVED *** source=%s claim=%02x%02x%02x%02x... approvals=%u",
                 source, claim_id[0], claim_id[1], claim_id[2], claim_id[3],
                 (unsigned)s_bitpiggy_approved_count);
        lcd_status_set(LCD_STATUS_APPROVED, 0, claim_id);
        s_blink_until_us = esp_timer_get_time() + 1800ULL * 1000ULL;
    } else {
        s_bitpiggy_rejected_count++;
        ESP_LOGI(TAG, "*** BITPIGGY NOT YET *** source=%s claim=%02x%02x%02x%02x... rejections=%u",
                 source, claim_id[0], claim_id[1], claim_id[2], claim_id[3],
                 (unsigned)s_bitpiggy_rejected_count);
        lcd_status_set(LCD_STATUS_REJECTED, 0, claim_id);
        s_blink_until_us = esp_timer_get_time() + 300ULL * 1000ULL;
    }

    s_bitpiggy_has_pending_claim = false;
    return true;
}
...
static int deck_init(void) {
    ...
    memset(&s_q_bitpiggy_chore_claim, 0, sizeof(s_q_bitpiggy_chore_claim));
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        switch (kind) {
            ...
            case DECK_KIND_BITPIGGY_CHORE_CLAIM: q = &s_q_bitpiggy_chore_claim; break;
            default: continue;
        }
        if (q->count >= sizeof(q->entries) / sizeof(q->entries[0])) continue;
        q->entries[q->count++] = e;
    }
    ESP_LOGI(TAG, "deck: %u entries; for-me: hb=%u tap=%u rule=%u conf_tap=%u chan_open=%u chan_commit=%u chan_close=%u scripted=%u offer=%u activate=%u bitpiggy_claim=%u",
             (unsigned)count,
             ...
             (unsigned)s_q_bitpiggy_chore_claim.count);
    return 0;
}
...
static void on_radio_recv(const uint8_t sender_mac[6],
    ...
    bool is_bitpiggy_approval  = false;
    bool is_bitpiggy_rejection = false;
    ...
    else if (memcmp(th, s_bitpiggy_approval_type_hash,  32) == 0) { kind = "bitpiggy_approval"; is_bitpiggy_approval = true; }
    else if (memcmp(th, s_bitpiggy_rejection_type_hash, 32) == 0) { kind = "bitpiggy_rejection"; is_bitpiggy_rejection = true; }

    ESP_LOGI(TAG, "RX [%s] %s verified (rx_total=%u)", mac_str, kind, (unsigned)s_rx_counter);


    // BitPiggy parent decision cells. Wallet signature verification above is
    // the v0 parent-capability gate. Payload layout from bitpiggy_cell_specs:
    // approval/rejection claim_id lives at offset 16.
    if (is_bitpiggy_approval || is_bitpiggy_rejection) {
        handle_bitpiggy_decision(is_bitpiggy_approval, payload, ptot, mac_str);
        return;
    }
...
static void broadcast_bitpiggy_chore_claim(void) {
    static uint32_t claim_counter = 0;
    const uint8_t *cell_flash, *sig_flash;
    if (!deck_pop(&s_q_bitpiggy_chore_claim, "bitpiggy_chore_claim", &cell_flash, &sig_flash)) return;
    ...
    if (cm_radio_send_cell(cell, sig, cell_id) == 0) {
        claim_counter++;
        const uint8_t *payload = cm_payload(cell);
        memcpy(s_bitpiggy_pending_claim_id, payload, 16);  // ChoreClaim.OFF_CLAIM_ID = 0
        s_bitpiggy_has_pending_claim = true;
        ESP_LOGI(TAG, "TX *** BITPIGGY CHORE CLAIM #%u *** (deck, broadcasting) cell_id=0x%08x claim=%02x%02x%02x%02x...",
                 (unsigned)claim_counter, (unsigned)cell_id,
                 s_bitpiggy_pending_claim_id[0], s_bitpiggy_pending_claim_id[1],
                 s_bitpiggy_pending_claim_id[2], s_bitpiggy_pending_claim_id[3]);
        lcd_status_set(LCD_STATUS_CLAIM, claim_counter, s_bitpiggy_pending_claim_id);
        // Local confirmation: a longer blink than tap so the claim is visible.
        s_blink_until_us = esp_timer_get_time() + 1200ULL * 1000ULL;
    }
}
...
static void inject_process_line(const char *line, size_t len) {
    ...
    const uint8_t *th = cm_type_hash(s_inject_cell);
    bool is_bitpiggy_approval =
        memcmp(th, s_bitpiggy_approval_type_hash, 32) == 0;
    bool is_bitpiggy_rejection =
        memcmp(th, s_bitpiggy_rejection_type_hash, 32) == 0;
    if (is_bitpiggy_approval || is_bitpiggy_rejection) {
        uint8_t hash[32];
        cm_sig_hash_cell(s_inject_cell, hash);
        if (cm_sig_verify(s_wallet_pubkey, hash, s_inject_sig) == 0) {
            handle_bitpiggy_decision(is_bitpiggy_approval,
                                     cm_payload(s_inject_cell),
                                     cm_payload_total(s_inject_cell),
                                     "serial");
        } else {
            ESP_LOGW(TAG, "inject: BitPiggy decision signature INVALID (wallet pubkey)");
        }
    }
...
static void *mesh_demo_thread(void *arg) {
    ...
    mbedtls_sha256((const unsigned char *)BITPIGGY_CHORE_CLAIM_TYPE_NAME,
                   sizeof(BITPIGGY_CHORE_CLAIM_TYPE_NAME) - 1,
                   s_bitpiggy_chore_claim_type_hash, 0);
    mbedtls_sha256((const unsigned char *)BITPIGGY_APPROVAL_TYPE_NAME,
                   sizeof(BITPIGGY_APPROVAL_TYPE_NAME) - 1,
                   s_bitpiggy_approval_type_hash, 0);
    mbedtls_sha256((const unsigned char *)BITPIGGY_REJECTION_TYPE_NAME,
                   sizeof(BITPIGGY_REJECTION_TYPE_NAME) - 1,
                   s_bitpiggy_rejection_type_hash, 0);
    ...
    lcd_status_set(LCD_STATUS_READY, 0, NULL);
    ...
    // BitPiggy: first chore claim after boot+12s, then every ~15s.
    uint64_t next_bitpiggy_claim_us = esp_timer_get_time()
                                    + 12000ULL * 1000ULL
                                    + (uint64_t)(esp_random() % 3000u) * 1000ULL;
    ...
    while (1) {
        ...
        // BitPiggy child-device demo: emit a pre-signed chore claim cell.
        if (now_us >= next_bitpiggy_claim_us) {
            broadcast_bitpiggy_chore_claim();
            next_bitpiggy_claim_us = now_us
                                   + 15000ULL * 1000ULL
                                   + (uint64_t)(esp_random() % 3000u) * 1000ULL;
        }
